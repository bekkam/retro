...
public abstract class InventoryItem implements Maintainable{
	...
	//method to return string list of inventory
	public String listProperties( )
	{
		Set< Object > keys = props.keySet(); // get property names
	    List <String> theList = new ArrayList<String>();
	    // save name/value pairs to list object
	    //so contents of file can be accessed outside of database (and sent to view)
	    for ( Object key : keys ){
	       theList.add("\n" + key + "\t" + props.getProperty( ( String ) key )  ); 
	    }
	      String stringList = "\n* Current List *" + convertListToPrint(theList) + "\n";
	      return stringList;
	   } // end method listProperties
	
	
	//helper method to format inventory list
	public String convertListToPrint(List <String> list){
		String result = "";
		   ...
		   public String addItem(String itemString){
				...
				//append slashes to beginning and end in order to manipulate fields with stringbuilder
				 getPropertyTable().setProperty(getFirstField(stringWithSlashes), stringWithSlashes);
		  		saveProperties();
		  		nextDialog = ("Added item '" + itemString + "' to inventory\n");
		  		return nextDialog;
		   }
	...
	//string builders
	   ...
	   public String editItem(int num, String key, String newField ){
	       loadProperties();
	      ...
	      String afterDeletingAtStartAndEnd = builder.toString(); 
	      ...
	      saveProperties();
	      String nextDialog = ("\nAfter saving update to inventory..." + listProperties());
	      return nextDialog;
	   }
		...
		public String deleteItem(String deleteString){
			...
			saveProperties();
			nextDialog = (deleteString + " was deleted from inventory\n");
			return nextDialog;
		}
		//class does not implement Maintainable methods searchItem; therefore it is declared abstract
}
